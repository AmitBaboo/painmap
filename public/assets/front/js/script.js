    ...
    var medlife = {
        ...
        bindEvents: function () {
            ...
            $(document).on(
                ...
                self.toggleHistory
            ); // Toggle History-policy
            $(document).on("click", ".yamm .dropdown-menu", function (e) {
                e.stopPropagation();
            }); //Single-Shop-Carousel Slider
        ...
        initContactForm: function () {
            ...
            contactForm.on("submit", function (e) {
                ...
                $("#cemail").html("");
                $("#cmessage").html("");
                $("#csuccessmessage").html(
                    `<b style="color: gold">Sending Message ...</b>`
                );
                ...
                var redirectToInput = self.find(".md__redirect-to"),
                    redirectTo =
                        typeof redirectToInput !== "undefined"
                            ? redirectToInput.val()
                            : "",
                ...
                if (data["g-recaptcha-response"].length === 0) {
                    $("#csuccessmessage").html(
                        `<b style="color: red">Please indicate you are not a robot.</b>`
                    );
                    ...
                    $.ajax({
                        ...
                        .done(function (response) {
                            ...
                            if (
                                ...
                                $("#csuccessmessage").html(
                                    `<b style="color: green">The message was submitted successfully, Thank you</b>`
                                );
                                setTimeout(() => {
                                    $("#csuccessmessage").html("");
                                    $("#contact_name").val("");
                                ...
                                responseWrapper
                                    .empty()
                                    .addClass("js-response-error")
                                    .append(
                                        "<p>An error occurred. Please try again in a few seconds.</p>"
                                    )
                                    .fadeIn();
                            }
                        })
                        .fail(function (txt, err) {
                            ...
                            // console.log(data);
                            console.log(txt);
                            let data = txt.responseJSON.errors;
                            data.name !== undefined
                                ? $("#cname").html(
                            ...
                            data.email !== undefined
                                ? $("#cemail").html(
                                      `<b style="color: red">${data.email}</b>`
                                  )
                                : $("#cemail").html("");
                            data.message !== undefined
                                ? $("#cmessage").html(
                                      `<b style="color: red">${data.message}</b>`
                                  )
                                : $("#cmessage").html("");
                            responseWrapper
                                .empty()
